...
public class SearchUserServiceDefault implements SearchUserService {

    private final UserQueryRepository userQueryRepository;

    @Override
    public List<UserResponse> searchByEmail(SearchQueryRequest request) {
        return userQueryRepository.searchByEmailUserFilter(request.query(), request.size().orElse(10))
                .stream()
    ...
    public List<UserResponse> searchByName(SearchQueryRequest request) {
        return userQueryRepository.searchByNameUserFilter(request.query(), request.size().orElse(10))
                .stream()
                .map(UserDto::toResponse)
                .toList();
    }
